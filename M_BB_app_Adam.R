...
players <- data.frame(
  name = c("Rj Horry", "Colin Ruffin", "Reggie Bass", "Markeith Browning II", 
           "Jordan Wildy", "Jaylon McDaniel", "Grant Redd", "Jadis Jones",
           "Jalen Bouknight", "Anais Futrell", "Aly Tounkara", "Nathan Johnson Jr.",
           "Alex Nokes", "Jeremiah Talton", "Beren Demer Ozdemir", "Clayton Jackson"),
  number = c(  0, 1, 2, 3,
               4, 5, 8, 10,
               11, 12, 13, 15,
               21, 24, 33, 34),
...
")
  ...
  unique_shot <- reactive({
    ...
    query <- "SELECT DISTINCT dribble_catch FROM m_basketball_game_chart_t WHERE event = 'Shot'"
  ...
  output$playerCourt <- renderPlot({
    ...
    # Only add shooting percentages to the legend if a single player is selected
    if (input$playerName != "All Players") {
      legend("topleft", legend = df_shots$shooting_percentage_label[1], 
             fill = "gold", title = "Player Shooting Percentages", bty = "n", cex = 0.9)
    }
  }, bg = "transparent")
}
